...
function createMenu($parentId, $groupType)
{
	...
	{	
		echo '<li>';
		?>
		<? $link=$groupRow['linkType']; ?>
    	<a href="<? if($link=='File' or $link=='Link'){ if($link=='File') echo CMS_FILES_DIR; echo $groupRow['contents']; }else{ echo $groupRow['urlname']; }?>" <? if($groupRow['linkType']=='Link' and $groupRow['urlname']!='home') echo 'target="_blank"';?>><?=$groupRow['name'];?></a>
		<?
